...
canvas = Canvas(root, width=505, height=550, bd=0, bg="Black") 
canvas.pack(padx=10, pady=10) 
score = Label(height=50, width=80, text="Score: 00", font="TimesNewRoman 14 bold")  
...
class Bricks:
    def __init__(self, canvas, color):
        self.canvas = canvas
        self.id = canvas.create_rectangle(5, 5, 25, 25, fill=color, width=2)
...
def start_game(event):
    ...
    if playing is False:
        ...
        canvas.delete("all")
        BALL_COLOR = ["white"]
        BRICK_COLOR = ["cyan", "light blue", "turquoise", "dark blue", "turquoise3"]
        ...
        while 1:
            if paddle.pausec !=1:
                ...
                if not ball.bottom_hit:
                    ...
                    root.update()
                    time.sleep(0.01)
                    if ball.hit==100: #winning score
                        canvas.create_text(250, 250, text="YOU WON!", fill="bisque", font="TimesNewRoman 24 bold")
                        root.update_idletasks()
                        root.update()
                        playing = False
                        break
                else:
                    canvas.create_text(250, 250, text="GAME OVER!", fill="red", font="TimesNewRoman 24 bold")
                    root.update_idletasks()
                    root.update()
                    playing = False
                    break
                ...
                try:
                    ...
                    m=canvas.create_text(250, 250, text="PAUSE!", fill="yellow", font="TimesNewRoman 24 bold")
                root.update_idletasks()
                root.update()


root.bind_all("<Return>", start_game) 
canvas.create_text(250, 250, text=" PRESS ENTER TO THE START THE GAME!", fill="light steel blue", font="TimesNewRoman 16 bold")
j=canvas.find_all()
root.mainloop()
